...
export default class Usdt extends React.Component {
  ...
  getYoData() {
    ...
    return action({url: 'https://yobit.net/api/3/depth/eth_usd'}).then(res => {
      let result = []
      const asks = res.eth_usd.asks
      const bids = res.eth_usd.bids
      for (let i = 0; i < 10; i++) {
        result.push({
          ask: asks[i][0],
          askMount: asks[i][1],
  ...
  getHuoData() {
    ...
    return action({
      url: 'https://api.huobipro.com/market/depth?symbol=ethusdt&type=step0'
    }).then(res => {
      let result = []
